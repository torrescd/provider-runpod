...
type ProviderCredentials struct {
	...
	// Only a Kubernetes Secret is accepted. Environment, filesystem, and
	// injected credentials are intentionally disabled to keep key provenance
	// auditable and namespaced.
...
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,provider,runpod}
...
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:printcolumn:name="SECRET-NAME",type="string",JSONPath=".spec.credentials.secretRef.name",priority=1
// +kubebuilder:resource:scope=Cluster,categories={crossplane,provider,runpod}
// A ClusterProviderConfig configures a RunPod provider.
