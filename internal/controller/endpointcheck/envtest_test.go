	...
	t.Run("generated CRDs enforce security and lifetime constraints", func(t *testing.T) {
		invalidTemplate := &serverlessv1alpha1.Template{
			...
			Spec: serverlessv1alpha1.TemplateSpec{ForProvider: serverlessv1alpha1.TemplateParameters{
				Name: "mutable-image", ImageName: "ghcr.io/example/model:latest",
			}},
		}
		...
		missingTemplate.Spec.ForProvider.TemplateID = ""
		...
		ambiguousCheck := validEnvtestCheck("ambiguous-endpoint", "team-a")
		ambiguousCheck.Spec.ForProvider.EndpointIDRef = &xpv2.Reference{Name: "endpoint"}
		if err := kube.Create(context.Background(), ambiguousCheck); !apierrors.IsInvalid(err) {
			t.Fatalf("EndpointCheck with two endpoint selectors should be rejected, got %v", err)
		}
