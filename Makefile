# ====================================================================================
# Setup Project
PROJECT_NAME := provider-runpod
PROJECT_REPO := github.com/torrescd/$(PROJECT_NAME)
...
GO_STATIC_PACKAGES = $(GO_PROJECT)/cmd/provider $(GO_PROJECT)/cmd/model-router
...
IMAGES = provider-runpod model-router
...
XPKGS = provider-runpod
-include build/makelib/xpkg.mk

# NOTE(hasheddan): we force image building to happen prior to xpkg build so that
# we ensure image is present in daemon.
xpkg.build.provider-runpod: do.build.images
...
dev: $(KIND) $(KUBECTL)
	...
	@$(INFO) Installing Provider RunPod CRDs
	@$(KUBECTL) apply -R -f package/crds
	@$(INFO) Starting Provider RunPod controllers
...
# This target prepares repo for your provider by replacing all "runpod"
